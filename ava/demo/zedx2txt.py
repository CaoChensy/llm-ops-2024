...
def parse_zedx(input_path, output_path, error_path, with_pic=1):
...
if __name__ == "__main__":
    paths = glob.glob("demo/dataset/multi/*")
    for path in paths:
        input_path = path
        output_path = f"demo/dataset/output/{path.split('/')[-1]}_with_pic"
        error_path = f"demo/dataset/output/{path.split('/')[-1]}.json"
        parse_zedx(input_path, output_path, error_path, with_pic=1)
